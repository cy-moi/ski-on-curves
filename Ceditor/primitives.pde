...
void conePts(pt P, pt Q, float p, float q, pt[] top, pt[] bottom) { // surface
  vec V = V(P,Q);
  vec I = U(Normal(V));
  vec J = U(N(I,V));
  circles(P,V,I,J,p,q, top, bottom);
  }

void circles(pt P, vec V, vec I, vec J, float r, float rd, pt[] top, pt[] bottom) {
  float da = TWO_PI/36;
  int counter = 0;
  for(float a=0; a<=TWO_PI; a+=da) {
    top[counter] = P(P,r*cos(a),I,r*sin(a),J,0,V); 
    bottom[counter] = P(P,rd*cos(a),I,rd*sin(a),J,1,V); 
    counter++;}
  }
...
void fan(pt P, vec V, vec I, vec J, float r, float size) {
  float da = TWO_PI/36;
  beginShape(TRIANGLE_FAN);
    v(P(P,V));
    for(float a=PI/2 + size; a<=(TWO_PI-PI/2-da-size); a+=da) {v(P(P,r*cos(a),I,r*sin(a),J)); fill(bw[int(a*100)%2]);}
  endShape();
  //println("hhahahahah");
}
void fan(pt P, vec V, float r, float size) {  
  vec I = U(Normal(V));
  vec J = U(N(I,V));
  fan(P,V,I,J,r, size);
  }

void fan1(pt P, vec V, vec I, vec J, float r) {
  float da = TWO_PI/36;
  beginShape(TRIANGLE_FAN);
    v(P(P,V));
    for(float a=PI/2; a<=(TWO_PI-PI/2-da); a+=da) {v(P(P,r*cos(a),I,r*sin(a),J));fill(bw[int(a*100)%2]);}
  endShape();
  }
  
void fan2(pt P, vec V, vec I, vec J, float r) {
  float da = TWO_PI/36;
  beginShape(TRIANGLE_FAN);
    v(P(P,V));
    for(float a=PI/2; a<=(TWO_PI-PI/2-da); a+=da) {v(P(P,r*cos(a),I,r*sin(a),J));fill(wb[int(a*100)%2]);}
  endShape();
  }

void fanbw(pt P, vec V, float r) {  
  vec I = U(Normal(V));
  vec J = U(N(I,V));
  fan1(P,V,I,J,r);
  }

void fanwb(pt P, vec V, float r) {  
  vec I = U(Normal(V));
  vec J = U(N(I,V));
  fan2(P,V,I,J,r);
  }
        ...
        translate(0,-d*n/2,0);
          for(int j=0; j<n; j++)
            ...
            pushMatrix();
              ...
              for(int i=0; i<n; i++)
                {
                fill(grey); box(d,d,1);  pushMatrix(); translate(d,d,0);  box(d,d,1); popMatrix();
                fill(white); pushMatrix(); translate(d,0,0); box(d,d,1); translate(-d,d,0); box(d,d,1); popMatrix();
                translate(2*d,0,0);
                }
            popMatrix();
            translate(0,2*d,0);
