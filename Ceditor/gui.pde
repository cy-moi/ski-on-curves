...
//  if(key=='`') picking=true; 
  ...
  if(key == 'r') showRunner = !showRunner;
  if(key == 'j') {jump = !jump;}
...
void displayHeader()  // Displays title and authors face on screen
    ...
    scribeHeader(footMessage, 50);
    fill(white); image(myFace, width-myFace.width/9,25,myFace.width/9,myFace.height/9); 
    }
...
String title ="3D curve editor", name ="Congyao Zheng",
       menu="?:help, !:picture, ~:(start/stop)capture, space:rotate, `/wheel:closer, t/T:target, a:anim, #:quit",
       guide="click&drag:pick&slide on floor, xz/XZ:move/ALL, e:exchange, q/p:copy, l/L:load, w/W:write, m:subdivide method, {: show Curve, o: show Offset, s: Character, r: show runner"; // user's guide